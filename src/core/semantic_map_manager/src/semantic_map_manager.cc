...
ErrorType SemanticMapManager::UpdateSemanticMap(
  ...
  if (agent_config_info_.enable_fast_lane_lut) {
    UpdateLocalLanesAndFastLut(); // 找到ego_state附近的车道线, 并将车道线合并为local_lane, 构建快速查找表
  }
...
ErrorType SemanticMapManager::NaiveRuleBasedLateralBehaviorPrediction(
  ...
  decimal_t prob_nl = 0.0;
  decimal_t prob_nr = 0.0;

  const decimal_t lat_distance_threshold = 0.4;
  const decimal_t lat_vel_threshold = 0.35;

  // add: Nudge判断条件
  const decimal_t nudge_lat_distance_threshold = 0.5;
  const decimal_t nudge_lat_vel_threshold = 0.5;


  // 右手坐标系, 向左距离 > 阈值 && 向左速度 > 阈值 && 左车道线可用 && 左车道线可变道 -> 预测行为: 向左变道
    ...
    if (fs.vec_dt[0] > lat_distance_threshold &&
      ...
      printf(
          ...
          vehicle.id(), nearest_lane_id, fs.vec_dt[0], fs.vec_dt[1]);
               ...
               nearest_lane.r_change_avbl) { // 右手坐标系, 向右距离 > 阈值 && 向右速度 > 阈值 && 右车道线可用 && 右车道线可变道 -> 预测行为: 向右变道
      prob_lcr = 1.0;
      printf(
          "[NaivePrediction]vehicle %d lane id %d, lat d %lf lat dd %lf, "
          "behavior "
          "lcr.\n",
          vehicle.id(), nearest_lane_id, fs.vec_dt[0], fs.vec_dt[1]);
    } else if (fs.vec_dt[0] > 0 && fs.vec_dt[0] <= nudge_lat_distance_threshold 
      && fs.vec_dt[1] > 0 && fs.vec_dt[1] <= nudge_lat_vel_threshold) {
      // 左Nudge：横向偏移较小且向右速度较小
      prob_nl = 1.0;
      printf("[NaivePrediction]vehicle %d nudge left.\n", vehicle.id());
    } else if (fs.vec_dt[0] < 0 && fs.vec_dt[0] >= -nudge_lat_distance_threshold 
      && fs.vec_dt[1] < 0 && fs.vec_dt[1] >= -nudge_lat_vel_threshold) {
      // 右Nudge：横向偏移较小且向左速度较小
      prob_nr = 1.0;
      printf("[NaivePrediction]vehicle %d nudge right.\n", vehicle.id());
    } else { // 预测行为: 保持
      prob_lk = 1.0;
    }
  } else { // 左手坐标系, 相反
...
ErrorType SemanticMapManager::UpdateSemanticLaneSet() {
  ...
  {
    for (const auto &pe : surrounding_lane_net_.lane_set) { // 循环每条车道线, 使用三次样条拟合采样点
      ...
      if (common::LaneGenerator::GetLaneBySamplePoints(
              samples, &semantic_lane.lane) != kSuccess) { // 使用Spline拟合采样点 (样条的自变量为弧长, 因变量为两轴的坐标X和Y)
        continue;
      }
...
ErrorType SemanticMapManager::UpdateKeyVehicles() {
  ...
  key_vehicles_.vehicles.clear();
  {
    // ~ 3. 遍历周围车辆，筛选符合条件的车辆
    for (const auto &v : semantic_surrounding_vehicles_.semantic_vehicles) {
      // 条件1：车辆距离车道中心线的横向距离不超过阈值
      if (v.second.dist_to_lane > max_distance_to_lane_) { // 2.0
        continue;
      }
      // 条件2：车辆所在车道是关键车道之一
