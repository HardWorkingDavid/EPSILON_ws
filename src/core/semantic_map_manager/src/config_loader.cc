...
ErrorType ConfigLoader::ParseAgentConfig(AgentConfigInfo *p_agent_config) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    if (agent["id"].get<int>() != ego_id_) continue; // 只读取主车的信息, ego_id_ = 0
    // common::GridMapMetaInfo构造函数中计算了w_metric和h_metric
    p_agent_config->obstacle_map_meta_info = common::GridMapMetaInfo(
        agent["obstacle_map_meta_info"]["width"].get<double>(),      // 1000, 索引数
    ...
    p_agent_config->enable_openloop_prediction =
        agent["enable_openloop_prediction"].get<bool>();             // true
