...
DataRenderer::DataRenderer(SemanticMapManager *smm_ptr)
  ...
  std::array<int, 2> map_size = {
      {obstacle_map_info_.height, obstacle_map_info_.width}}; // 1000, 1000
  std::array<decimal_t, 2> map_resl = {
      {obstacle_map_info_.resolution, obstacle_map_info_.resolution}}; // 0.2, 0.2
...
ErrorType DataRenderer::FakeMapper() {
  ...
  for (auto it = obs_grids_.begin(); it != obs_grids_.end();) {
    ...
    if (dx >= dist_thres || dy >= dist_thres) { // 滤掉距离过远的障碍物
      ...
      p_obstacle_grid_->SetValueUsingGlobalPosition( // SCANNED_OCCUPIED表示raycasting得到的障碍物
...
ErrorType DataRenderer::GetSurroundingVehicles(
  ...
  for (const auto &v : vehicle_set.vehicles) {
    if (v.second.id() == ego_id_) continue; // 跳过ego_car
    // 计算距ego_car的距离
    double dx =
