...
void Visualizer::VisualizeIntentionPrediction(
  ...
  for (const auto &p_sv : semantic_vehicles.semantic_vehicles) {
    ...
    for (const auto &entry : semantic_vehicle.probs_lat_behaviors.probs) {
      ...
      common::ColorARGB color = common::cmap.at("yellow");  // 默认颜色

      if (beh == common::LateralBehavior::kLaneChangeRight) {
        angle_offset = -kPi / 2.0;
        color = common::cmap.at("red");
      } else if (beh == common::LateralBehavior::kLaneChangeLeft) {
        angle_offset = +kPi / 2.0;
        color = common::cmap.at("blue");
      } else if (beh == common::LateralBehavior::kLaneKeeping) {
        angle_offset = 0.0;
        color = common::cmap.at("green");
      }
      // 添加Nudge行为的处理
      else if (beh == common::LateralBehavior::kNudgeLeft) {
        angle_offset = +kPi / 6.0;  // 左Nudge：小角度偏移（30度）
        length = prob * 1.0;  // 箭头更短（区别于变道）
        color = common::cmap.at("cyan");  // 青色（独特颜色）
      } else if (beh == common::LateralBehavior::kNudgeRight) {
        angle_offset = -kPi / 6.0;  // 右Nudge：小角度偏移（-30度）
        length = prob * 1.0;
        color = common::cmap.at("magenta");  // 品红（独特颜色）
      }
