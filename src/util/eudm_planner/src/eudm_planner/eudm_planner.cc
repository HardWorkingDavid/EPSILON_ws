...
ErrorType EudmPlanner::RunEudm() {
  ...
  std::vector<std::thread> thread_set(n_sequence); // 创建27个线程
  PrepareMultiThreadContainers(n_sequence);        // 准备多线程容器
...
ErrorType EudmPlanner::StrictSafetyCheck(
  ...
  for (auto it = surround_trajs.begin(); it != surround_trajs.end(); it++) {
    ...
    for (int i = 0; i < num_points_ego; i++) {
      ...
      // 给自车与目标车的长宽加0.1的偏移量，防止碰撞检测时出现误差
...
// 5 代价评估:  策略的总体奖励是由每个cfb选择的场景的奖励加权和计算得出的，
// 每个场景的奖励函数主要由效率成本，安全成本，导航成本的线性组合组成
